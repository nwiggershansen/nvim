...
local base_keymaps = function(bufnr)
  ...
  vim.keymap.set('n', '<C-f>', function() require("conform").format({ async = true }) end, opts)
  vim.keymap.set('n', '<leader>fm', function() require("conform").format({ async = true }) end, opts)
